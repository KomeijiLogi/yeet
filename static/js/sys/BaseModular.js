$(function(){
	//角色列表
	easyExt.initDataGrid('#dg','/t_base_modular/findAllByPage');
	//easyExt.initTreeGrid('#tg','/sys_privilege/findAll'); 
	//删除角色
	$("#del").click(function(){
		var selRows=$('#dg').datagrid('getSelections');
		easyExt.del(selRows,'/t_base_modular/deleteBatch',function(){//删除成功后执行的动作，一般用于刷新datagrid
			$('#dg').datagrid('reload'); 
	...
	//添加角色
	$("#add").click(function(){
		easyExt.add('/t_base_modular/addOne',function(){
			$('#dg').datagrid('reload'); 
		});
	});
	//修改角色
	$("#edit").click(function(){
		var selRows=$('#dg').datagrid('getSelections');
		easyExt.edit(selRows,'/t_base_modular/updateOne',function(){//删除成功后执行的动作，一般用于刷新datagrid
			$('#dg').datagrid('reload'); 
		});
	});
	//保存权限
		...
		$('#userRole').dialog({
			...
			buttons:[{
					...
					handler:function(){
						...
						easyExt.ajax({
							url:easyExt.url+"/t_base_modular/saveModluarMenu?modularId="+row.id+"&menuIds="+pIds},
...
function lookP(id){
	...
	easyExt.ajax({url:easyExt.url+"/t_base_modular/getPrivilege?id="+id,type:'GET'},function(data,status, xhr){
...
function savePermission(){
	...
	easyExt.ajax({url:easyExt.url+"/t_base_modular/saveRolePrivilege?roleId="+c[0].id+"&privilegeIds="+pIds},function(data, status, xhr){
		if(data){
